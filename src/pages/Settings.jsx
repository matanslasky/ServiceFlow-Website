...
import { ArrowLeft, Lock, Mail, Save, LogOut, Bot, Sliders, FileText, Briefcase } from 'lucide-react';
      ...
      if (user) {
        ...
        if (data) {
          ...
          const { data: newData } = await supabase
            .from('agent_settings')
            .insert([{ user_id: user.id, agent_name: 'Assistant', agent_tone: 'Professional' }])
            .select().single();
          if (newData) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <div className="p-6 md:p-12 max-w-5xl mx-auto">
        <h1 className="text-3xl font-extrabold text-slate-900 mb-8">Agent Configuration</h1>
        ...
        <form onSubmit={handleUpdateProfile} className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 outline-none bg-slate-50 focus:bg-white transition-all" value={agentName} onChange={(e) => setAgentName(e.target.value)} placeholder="e.g. Sarah" />
                </div>
                <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Job Title</label>
                    <div className="relative">
                        <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                        <input type="text" className="w-full pl-10 pr-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 outline-none bg-slate-50 focus:bg-white transition-all" value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} placeholder="e.g. Medical Receptionist" />
                    </div>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 outline-none bg-slate-50 focus:bg-white transition-all h-24 resize-none" value={businessContext} onChange={(e) => setBusinessContext(e.target.value)} placeholder="e.g. We are a dermatology clinic specializing in skincare. We are closed on Sundays." />
                <p className="text-xs text-slate-400 mt-2">The AI will use this context when drafting emails.</p>
              </div>
          ...
          <div className="space-y-8">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><Sliders className="text-slate-400"/> Tone & Style</h2>
                <div className="space-y-4">
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider">Primary Tone</label>
                    <div className="grid grid-cols-2 gap-3">
                    {['Professional', 'Friendly', 'Empathetic', 'Urgent'].map((tone) => (
                        <button key={tone} type="button" onClick={() => setAgentTone(tone)} className={`py-2 px-3 rounded-lg text-sm font-medium border transition-all ${agentTone === tone ? 'bg-teal-600 text-white border-teal-600 shadow-md' : 'bg-white border-slate-200 text-slate-600 hover:border-teal-300'}`}>{tone}</button>
                    ))}
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><Lock className="text-slate-400"/> Account</h2>
                <div className="space-y-4">
                    ...
                    <button type="submit" className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg flex items-center justify-center gap-2"><Save size={18} /> Save Changes</button>
                    <button type="button" onClick={async () => { await supabase.auth.signOut(); navigate('/'); }} className="w-full text-red-500 hover:text-red-700 text-sm font-bold mt-4">Sign Out</button>
                </div>
            </div>
          </div>
